    ...
    with gr.Row():
        with gr.Column(scale=1, elem_classes="sidebar-panel"):
            ...
            session_selector = gr.Dropdown(
                choices=["Chat-Default"], value="Chat-Default", label="Select Conversation Thread", interactive=True, container=False
            )
        with gr.Column(scale=3, elem_classes="chat-panel"):
            gr.Markdown("# 🔮 Nexus Persistent Assistant")
            chatbot = gr.Chatbot(label="Conversation Window", elem_id="chat-box", height=550)
            with gr.Row():
                txt_input = gr.Textbox(placeholder="Ask a technical query...", container=False, scale=8)
                submit_btn = gr.Button("Send", variant="secondary", scale=1)

    submit_btn.click(fn=user_message, inputs=[txt_input, chatbot], outputs=[txt_input, chatbot], queue=False).then(fn=stream_response, inputs=[chatbot, active_thread_id], outputs=chatbot)
    txt_input.submit(fn=user_message, inputs=[txt_input, chatbot], outputs=[txt_input, chatbot], queue=False).then(fn=stream_response, inputs=[chatbot, active_thread_id], outputs=chatbot)
    new_chat_btn.click(fn=start_new_chat, inputs=[all_threads], outputs=[chatbot, all_threads, session_selector, active_thread_id])
    session_selector.change(fn=load_selected_chat, inputs=[session_selector], outputs=[chatbot, active_thread_id])
